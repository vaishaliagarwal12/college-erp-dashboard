...
function FacultyTable({
  ...
  const courseCount = (teacher) =>
    courses.filter((c) => c.instructor === teacher.name).length;
  ...
  if (faculty.length === 0) {
    return (
      <div className="rounded-xl border border-outline-variant bg-surface-container-lowest shadow-[0px_1px_3px_rgba(0,0,0,0.05),0px_10px_15px_-3px_rgba(0,0,0,0.02)] dark:border-gray-700/60 dark:bg-gray-900">
        <EmptyState
          message={hasFilters ? "No faculty match your search" : "No faculty found"}
          hint={
            hasFilters
          ...
          <tbody>
            {faculty.map((teacher, index) => (
              ...
              >
                ...
                <td className="px-4 py-4 text-sm text-on-surface-variant dark:text-gray-400">
                  {courseCount(teacher)}{" "}
                  {courseCount(teacher) === 1 ? "Course" : "Courses"}
                </td>
                ...
                <td className="px-4 py-4">
                  <div className="flex items-center justify-end gap-2 opacity-0 transition-opacity group-hover:opacity-100">
                    <button
                      onClick={() => navigate(`/faculty/details/${teacher._id}`)}
                    ...
                    {isAdmin && (
                      <button
                        onClick={() => navigate(`/faculty/edit/${teacher._id}`)}
                        className="p-1 text-on-surface-variant transition-colors hover:text-secondary dark:text-gray-400"
                        title="Edit Record"
